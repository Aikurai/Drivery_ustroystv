...
#define DRIVER_NAME "mai_practice8_driver"
...
static int pci_driver_probe(struct pci_dev *dev,
...
static int pci_driver_probe(struct pci_dev *dev,
			    const struct pci_device_id *ent)
{
	pr_info(DRIVER_NAME ": pci_driver_probe\n");
	...
	if ((result = register_netdev(fake_netdev))) {
		pr_info(DRIVER_NAME
			": pci_driver_probe: Failed to register_netdev: %d\n",
			result);
...
static void pci_driver_remove(struct pci_dev *dev)
{
	pr_info(DRIVER_NAME ": pci_driver_remove\n");
	if (fake_netdev) {
...
static int fake_net_dev_open(struct net_device *dev)
{
	pr_info(DRIVER_NAME ": fake_net_dev_open\n");
	return 0;
}

static int fake_net_dev_release(struct net_device *dev)
{
	pr_info(DRIVER_NAME ": fake_net_dev_release\n");
...
static ssize_t io_address_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	pr_info(DRIVER_NAME ": io_address_show\n");
